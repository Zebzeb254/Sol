...
class LogBot(irc.IRCClient):
    ...
    def action(self, user, channel, msg):
        ...
        elif re.search(r'\b(sie|xie|hir|pomf|furaffinity|furry|mounts|pelvis|climax|breast|kawaii|silicone|circumsision|sheathe|arse|cum|spew|penis|knot|vulpine|fart|tentacle|cock|butt|ass|yiff)\b', msg, re.I) and  re.search(r'%s' % self.nickname, msg, re.I):
            f = open("catText/catYiffed.txt","r")
            data = f.readlines()
            num_lines = sum(1 for line in data) - 1
            interact = data[random.randint(1, num_lines)].rstrip('\n')
